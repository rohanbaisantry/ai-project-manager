from app.common.entities import (CreateTeamMemberEntity, SaveNewChatEntity,
                                 SignupEntity)
...
async def login(mobile: str) -> CompanyGlobalDataSchema:
    use_cases = CommonUseCases()
    user, company, team_members, tasks = (
...
async def create_team_member(
    ...
    return serialize_user(team_member)
...
@router.post("/tasks")
async def create_task(data: CreateTaskEntity) -> list[UserSchema]:
    use_cases = CommonUseCases()
    team_members = await use_cases.create_task(data)
    return [serialize_user(item) for item in team_members]


@router.post("/tasks/{task_id}")
async def edit_task(
    task_id: PydanticObjectId, data: UpdateTaskEntity
) -> list[UserSchema]:
    use_cases = CommonUseCases()
...
async def new_chat_received(
    ...
    use_cases = CommonUseCases()
    system_response = await use_cases.save_new_chat_and_get_response(user_id, data)
